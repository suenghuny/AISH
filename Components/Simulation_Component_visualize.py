            ...
            for bullet in self.bullets:
                ...
                if target.status != 'destroyed':
                    if ((bullet.position_x - target_position_x) ** 2 +
                        ...
                        if (self.launcher.side == 'blue') and (target.cla == 'SSM') and (target.status == 'destroyed'):
                            self.env.monitor.destroying_distance_record.append(((
                                                                              self.launcher.position_x - target.position_x) ** 2 + (
                                                                              self.launcher.position_y - target.position_y) ** 2) ** 0.5/10)
                        else:pass
...
class Missile:
    ...
    def destroying(self, enemies, ships, flying_ssms_friendly, flying_ssms_enemy, flying_sams_friendly,
        ...
        if (self.launcher.side == 'blue')and (self.target.cla == 'SSM') and (self.target.status == 'destroyed'):
            ...
            self.env.monitor.destroying_distance_record.append(((
                                                                        self.launcher.position_x - self.target.position_x) ** 2 + (
                                                                        self.launcher.position_y - self.target.position_y) ** 2) ** 0.5/10)
