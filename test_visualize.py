...
def evaluation(agent, env, e, t):
    ...
    k = 0
    visual_memory_temp = np.zeros([100, cfg.discr_n])
    print(t)
    while not done:
            ...
            for i in range(len(env.friendlies_fixed_list)):
                ...
                if k >=2.0:
                    ...
                    data_memory.append([
                        ...
                        td_target])
                    if (t == 82) or (t==280):
                        print(ship_feature[0][7: 7+cfg.discr_n])
    ...
    for dataset in datasets:
        ...
        for e in range(200):
            env = modeler(data,
                          ...
                          action_history_step=cfg.action_history_step,
                          monitor = monitor)
            episode_reward, win_tag, t = evaluation(agent, env, e, t)
            #print(set(monitor.destroying_distance_record))
            # if e >= 20:
            #     history = list(set(monitor.destroying_distance_record))
            #     plt.figure(figsize=(10, 6))
            #     plt.hist(history, bins=30, edgecolor='black')
            #
            #     # 그래프 제목과 축 레이블 설정
            #     plt.title('Histogram of Destroying Distances')
            #     plt.xlabel('Distance')
        ...
        # with open("visual feature final random.json", "w", encoding='utf-8') as json_file:
        #     json.dump(visual_memory, json_file, ensure_ascii=False)


        history = list(set(monitor.destroying_distance_record))
        with open("history_rl.json", "w", encoding='utf-8') as json_file:
            json.dump(history, json_file, ensure_ascii=False)


        #color_choice = input("히스토그램 색상을 선택하세요 (blue/green): ").lower()

        # with open("state feature final rl.json", "w", encoding='utf-8') as json_file:
        #     json.dump(data_memory, json_file, ensure_ascii=False)
        #
        # with open("graph feature final rl.json", "w", encoding='utf-8') as json_file:
        #     json.dump(graph_memory, json_file, ensure_ascii=False)
